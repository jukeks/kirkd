...
class ClientMessageTest : FunSpec({
    test("parsing NICK") {
        val msg = "NICK test"
        val parsed = ClientMessage.parse(msg) as Message.Nick
    ...
    test("parsing USER") {
        ...
        val parsed = ClientMessage.parse(msg) as Message.User
    ...
    test("parsing PRIVMSG") {
        ...
        val parsed = ClientMessage.parse(msg) as Message.Privmsg
    ...
    test("parsing JOIN") {
        ...
        val parsed = ClientMessage.parse(msg) as Message.Join
        parsed.channel shouldBe "#channel"
    }

    test("parsing PART") {
        val msg = "PART #channel"
        val parsed = ClientMessage.parse(msg) as Message.Part
        parsed.channel shouldBe "#channel"
    }

    test("parsing PING") {
        val msg = "PING 123"
        val parsed = ClientMessage.parse(msg) as Message.Ping
        parsed.id shouldBe "123"
    }

    test("parsing PONG") {
        val msg = "PONG 123"
        val parsed = ClientMessage.parse(msg) as Message.Pong
        parsed.id shouldBe "123"
    }

    test("parsing QUIT") {
        val msg = "QUIT :bye"
        val parsed = ClientMessage.parse(msg) as Message.Quit
        parsed.message shouldBe "bye"
    }

    test("parsing TOPIC") {
        val msg = "TOPIC #channel :new topic"
        val parsed = ClientMessage.parse(msg) as Message.Topic
        parsed.channel shouldBe "#channel"
    ...
    test("parsing CAP") {
        ...
        val parsed = ClientMessage.parse(msg) as Message.Cap
